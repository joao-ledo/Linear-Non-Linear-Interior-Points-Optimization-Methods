...
function result = Newton(d,variavel)    
  a = -1.0;
  b = 1.0;
  E=0.2;
  k = 1;
  metodo.nome = 'Newton';
  metodo.x(k)=a; 
  metodo.derivadas{k} = deriva_objetivo(metodo.x(k),d,variavel);
  metodo.x(k+1) = b;
  ...
  metodo.numero_iteracoes = k;
  result = max(metodo.x(k));
end

function result = deriva_objetivo(alpha,direcao, x)  
    x1 = x(1) + alpha*direcao(1);
    x2 = x(2) + alpha*direcao(2);
    result = [2*x1+x2-3;2*x2+x1];
end
